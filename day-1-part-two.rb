...
def process_line(str)
  first_num = first_int(str.chars)
  last_num = last_int(str.chars)
  "#{first_num}#{last_num}".to_i
end

def first_int(arr)
  arr.each_with_index do |str, i|
    integer = int_or_nil(str)
    return integer if integer

    digit = word_digit_or_nil(arr, i)
    return digit if digit
  end
  raise "No first int found"
end
...
def last_int(arr)
  ...
  arr.reverse.each do |str|
    ...
    digit = word_digit_or_nil(arr, index)
    return digit if digit
    index -= 1
  end
  raise "No last int found"
end
...
def word_digit_or_nil(chars, index)
  word = ''
  for i in index..chars.length - 1
    word += chars[i]
    digit = parse_word_to_digit(word)
    return digit if digit
  end
  nil
end

def parse_word_to_digit(word)
  case word
  when 'one'
    return 1
  when 'two'
